...
#' @export
diagnosis <- function(content, measure, year, pmid, sortby, lefthand,
                      righthand, type, cofactorlabel, topic, theme) {
  temp <- content
  ...
  myframe$TP<-as.numeric(as.character(stringr::str_trim(myframe$TP)))
  myframe$FP<-as.numeric(as.character(stringr::str_trim(myframe$FP)))
  myframe$FN<-as.numeric(as.character(stringr::str_trim(myframe$FN)))
  myframe$TN<-as.numeric(as.character(stringr::str_trim(myframe$TN)))
  msg <- ""
  ...
  meta1 <- mada::madad(TP = myframe$TP, FN = myframe$FN, TN = myframe$TN,
                       FP = myframe$FP, names = myframe$Study,
                       data = myframe, correction = 0.5,
  ...
  style_font   <-   'font-family:Arial, Helvetica, sans-serif'
  style_bold   <-   'style="font-weight: bold"'
  style_normal <- 'style="font-weight:normal"'

  #' Start of SVG
  height = 265 + length(myframe$Study) * 20
  svgtext = paste('<?xml version="1.0" encoding="UTF-8"?>',
                    '<svg x="0" y="0" width="700px" height="',
                       height, '"px"',
  ...
  for(i in 1: length(myframe$Study)) {
    #' Citation
    studysize[i] = myframe$TP[i] + myframe$FP[i] + myframe$FN[i] + myframe$TN[i]
    withoutcome[i] = myframe$TP[i] + myframe$FN[i]
    prevalence[i] = round(100*(myframe$TP[i] + myframe$FN[i])/studysize[i],0)
    ...
    #' text
    svgtext <- paste(svgtext,
       '<text x="450" y="', 15 + i*20 , '" fill="black" style="font-weight:normal">', round(meta1$spec[[1]][i]*100,0), ' (', round(meta1$spec$spec.ci[i,1]*100,0), ' - ', round( meta1$spec$spec.ci[i,2]*100,0), ')</text>',
      sep = "")

    #' point estimate
    svgtext <- paste(svgtext,
      '<circle cx="', x, '" cy="' , 10 + i*20 ,'" r="3" stroke-width="0" style="fill:black;"/>',
       sep = "")
    ...
    svgtext <- paste(svgtext,
      '<line x1="', cl.lower ,'" y1="', 10 + i*20 ,'" x2="', ci.upper, '" y2="' , 10 + i*20 , '" style="stroke:rgba(0,0,0,1); stroke-width:2" />',
       sep = "")
  }

  #' Bottom of plots
  svgtext <- paste(svgtext,
    '<!-- Bottom of plot--><!-- Axes --><g style="stroke:rgba(0,0,0,0.2);stroke-width:2"><line x1="325" y1="', 25 + i*20, '" x2="425" y2="', 25 + i*20 ,'" />',
    sep = "")
  svgtext <- paste(svgtext,
     '<line x1="550" y1="', 25 + i*20, '" x2="650" y2="', 25 + i*20 ,'" /></g>',     sep = "")
  svgtext <- paste(svgtext,
     '<!-- Ticks --><g stroke="rgba(0,0,0,0.2)" stroke-width="1" fill="rgba(0,0,0,0.2)"><g transform="translate(400, 245) rotate(180)"><path d="M 0,0 L 0,5 M 50,0 L 50,5 M 100,0 L 100,5" /></g><g transform="translate(650, 245) rotate(180)"><path d="M 0,0 L 0,5 M 50,0 L 50,5 M 100,0 L 100,5" /></g></g>',
     sep = "")
  ...
  if (type=="ignore") {
    ...
    #' Sens
    ci.l <- round(100*boot::inv.logit(meta2$coef[2,2] - 1.96*meta2$coef[2,3]),0)
    ci.u <- round(100*boot::inv.logit(meta2$coef[2,2] + 1.96*meta2$coef[2,3]),0)
    ...
    #' Spec
    ci.l <- round(100*boot::inv.logit(meta2$coef[3,2] - 1.96*meta2$coef[3,3]),0)
    ci.u <- round(100*boot::inv.logit(meta2$coef[3,2] + 1.96*meta2$coef[3,3]),0)
    ...
    #' AUC
    auc     <- mada::AUC(mada::phm(myframe))
    svgtext <- paste(svgtext,
       '<text x="10" y="' , 95 + i*20 ,'" style="font-weight:bold">Area under the ROC curve: ', round(auc$AUC[[1]][1],3), '</text>',
       sep = "")
    ...
    #' Method 3
    #' madauni
    meta3 <- mada::madauni(myframe, correction = 0.5,
                           correction.control = "all")
    ...
    I2 <- max(I2,0)
    svgtext <- paste(svgtext,
      '<text x="10" y="' , 115 + i*20 , '"', style_bold, ">", "Heterogeneity: ", round(I2,1), "%</text>",
       sep = "")
    svgtext <- paste(svgtext,
      '<text x="10" y="', 130 + i*20 ,'"', style_normal, ">(For diagnostic odds ratio: Q = ",round(meta3$CQ['Q'],1),", DF = ",meta3$CQ['df'],")</text>",
      sep = "")

    #' Predictive values
    svgtext <- paste(svgtext, '<text x="10" y="' , 150 + i*20 ,'"', style_bold, ">Predictive values:</text>",sep="")
    totalstudied <- sum(myframe$TP) + sum(myframe$FP) + sum(myframe$FN) +
                   sum(myframe$TN)
    pooledprevalence <- (sum(myframe$TP) + sum(myframe$FN))/(totalstudied)
    ...
    #' About the studies
    svgtext <- paste(svgtext,
       "<!-- About the studies --><text x=\"10\" y=\"" , 225 + i*20 ,"\" style=\"font-weight:bold\">About the studies:</text>", sep = "")
    svgtext <- paste(svgtext,
       "<text x=\"10\" y=\"" , 245 + i*20 ,"\">", totalstudied ," persons (median ", median(studysize) ,", range ", min(studysize), " - ", max(studysize), ") in ", length(myframe$Study) ," studies.</text>", sep = "")
    svgtext <- paste(svgtext,
      "<text x=\"10\" y=\"", 260 + i*20 , "\">", sum(withoutcome), " persons with the outcome (median ", median(withoutcome), ", range ", min(withoutcome), " - ", max(withoutcome), ").</text>", sep = "")
  }
